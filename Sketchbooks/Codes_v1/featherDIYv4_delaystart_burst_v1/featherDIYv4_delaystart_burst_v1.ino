...
// ===========================
// USER INPUTS (for delayed start burst)
// ===========================
float sampleFreq = 1; // Sampling frequency in Hz
float writeMinutes = 0.25; // Number of minutes to sample
float sleepMinutes = 0.5; // Number of minutes to sleep
const int startYear = 2025; // Year to start sampling
const int startMonth = 4; // Month to start sampling
const int startDay = 21; // Day to start sampling
const int startHour = 11; // Hour to start sampling (24-hr format)
const int startMinute = 7; // Minute to start sampling
...
// ===========================
void loop() {
  // Time to begin? (User-defined parameters)
  ...
  // Not yet time to begin sampling
    ...
    // Blink green LED while waiting for sampling start
    waitingBlinks = waitingBlinks + 1;
    if (waitingBlinks <= 20) { // Lasts approximately 20s (since delay(500) is ~1s)
      digitalWrite(13, HIGH);
